...
export default async function Schedule() {
  ...
  const { success, gameData, isActivePlayer, participantsData } = await getNextUpcomingGame()
    ...
    <div className="flex flex-col gap-6">
      ...
      {gameData && success ? (
        <section className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700/60 dark:bg-zinc-900">
          <p className="mb-4 text-center text-[11px] font-semibold uppercase tracking-[0.15em] text-zinc-400 dark:text-zinc-500">Next Game Starts In</p>
          ...
          <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
            ...
            {gameData && participantsData && (
              <span className="text-xs tabular-nums text-zinc-500 dark:text-zinc-400">
                <span className="font-semibold text-zinc-700 dark:text-zinc-200">{12 - participantsData.length}</span> spots left
              </span>
            )}
            <Link href={`/game-status/${gameData.game_id}`} className="text-xs font-medium text-basket-400 transition-colors hover:text-basket-300">
              View Details
            </Link>
          </div>
        </section>
        ...
        <section className="rounded-xl border border-zinc-200 bg-white p-8 text-center dark:border-zinc-700/60 dark:bg-zinc-900">
          <p className="text-sm text-zinc-500 dark:text-zinc-400">No upcoming games scheduled yet.</p>
          {session?.user?.role === 'ADMIN' || session?.user?.role === 'ORGANIZER' ? (
            <Button variant="outline" asChild className="mt-4 border-zinc-200 text-xs dark:border-zinc-700">
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-zinc-500 dark:text-zinc-400">Available</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full bg-green-500" />
          <span className="text-[11px] text-zinc-500 dark:text-zinc-400">Registered</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full bg-red-500" />
          <span className="text-[11px] text-zinc-500 dark:text-zinc-400">Cancelled</span>
        </div>
      </div>
      ...
      {upcomingGamesSuccess && allGames && allGames.length > 0 ? (
        ...
        <section className="rounded-xl border border-zinc-200 bg-white p-8 text-center dark:border-zinc-700/60 dark:bg-zinc-900">
          <p className="text-sm text-zinc-500 dark:text-zinc-400">No games in the calendar yet.</p>
          {session?.user?.role === 'ADMIN' || session?.user?.role === 'ORGANIZER' ? (
            <Button variant="outline" asChild className="mt-4 border-zinc-200 text-xs dark:border-zinc-700">
