...
export default async function MembershipsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Section header */}
      <div>
        <h2 className="text-[11px] font-semibold uppercase tracking-[0.15em] text-zinc-500 dark:text-zinc-400">League Memberships</h2>
        <p className="mt-1 text-[13px] text-zinc-400 dark:text-zinc-500">View and manage your active memberships</p>
      </div>
      ...
      {!success || memberships.length === 0 ? (
        <div className="flex flex-col items-center py-12">
          ...
          <Link href="/leagues" className="mt-3 text-[13px] text-basket-400 transition-colors hover:text-basket-300">
            Browse leagues
          </Link>
        </div>
      ) : (
