...
export default async function AdminLocationsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const { page, pageSize, search, city, isActive } = searchParamsSchema.parse(resolvedSearchParams)
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-semibold text-zinc-800 dark:text-zinc-100">Locations</h1>
        ...
        <Button variant="outline" asChild className="h-8 border-zinc-200 text-xs dark:border-zinc-700">
          <Link href="/dashboard/locations/new" className="flex items-center gap-1.5">
      ...
      <AdminLocationsList locations={locations} cities={cities} totalLocations={totalLocations} pageSize={currentPagesize} />
