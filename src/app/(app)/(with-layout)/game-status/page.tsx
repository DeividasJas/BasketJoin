      ...
      <div className="flex flex-col gap-2.5">
        {gameData.game_date && (
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm text-zinc-700 dark:text-zinc-300">{format(gameData.game_date, 'EEEE, MMM d · h:mm a')}</span>
          </div>
        )}
        {gameData.location?.name && (
          <div className="flex items-center gap-2.5">
            <MapPin className="h-3.5 w-3.5 text-zinc-400 dark:text-zinc-500" />
            <span className="text-sm text-zinc-500 dark:text-zinc-400">{gameData.location.name}</span>
          </div>
        )}
      </div>
