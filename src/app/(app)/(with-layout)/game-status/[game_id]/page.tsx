...
export default async function DynamicGameStatusPage({ params }: { params: Promise<{ game_id: string }> }) {
  const session = await auth()
  const isLoggedIn = !!session?.user

  const gameId = Number((await params).game_id)
  const response = await getGameByIdAndLocation(Number(gameId))
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col gap-6">
      {/* Header */}
      <p className="text-center text-[11px] font-semibold uppercase tracking-[0.2em] text-zinc-400 dark:text-zinc-500">Next game in</p>
      ...
      <div className="flex flex-col gap-2.5">
        {gameDate && (
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm text-zinc-700 dark:text-zinc-300">{format(gameDate, 'EEEE, MMM d · h:mm a')}</span>
          </div>
        )}
        {locationName && (
          <div className="flex items-center gap-2.5">
            <MapPin className="h-3.5 w-3.5 text-zinc-400 dark:text-zinc-500" />
            <span className="text-sm text-zinc-500 dark:text-zinc-400">{locationName}</span>
          </div>
        )}
      </div>
