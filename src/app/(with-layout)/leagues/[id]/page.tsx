...
export default async function LeagueDetailPage({
  ...
  const canJoin =
    !isMember && (league.status === "ACTIVE" || league.status === "UPCOMING");
            ...
            <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-sm text-zinc-600 dark:text-zinc-300">
                  Pro-rated fee:{" "}
                  <span className="font-semibold tabular-nums text-basket-400">
                    {formatCurrency(proRatedCalculation.totalAmount)}
                  </span>
                </p>
                <p className="mt-0.5 text-[11px] tabular-nums text-zinc-400 dark:text-zinc-500">
                  {proRatedCalculation.percentageOfSeason.toFixed(0)}% remaining
                  &middot; {proRatedCalculation.schedules.length} payment
        ...
        <section className="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700/60 dark:bg-zinc-900">
          ...
          <div className="mt-4 flex flex-col gap-4">
            <div>
              ...
              <p className="mt-0.5 text-[11px] tabular-nums text-zinc-400 dark:text-zinc-500">
                Full season &middot; {paymentDueDates.length} installments
              </p>
            </div>
