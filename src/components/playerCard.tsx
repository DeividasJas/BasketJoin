...
export default function PlayerCard({ player }: { player: Users }) {
  const name = [player.given_name, player.family_name]
    .filter(Boolean)
    .join(" ");
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <span className="max-w-[72px] truncate text-center text-[11px] leading-tight text-zinc-600 dark:text-zinc-400">
        {name}
      </span>
    </div>
  );
