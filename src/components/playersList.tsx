...
function MilestoneMarker({
  ...
  return (
    ...
    >
      ...
      <span className={`text-[10px] font-medium uppercase tracking-[0.15em] ${textColor}`}>
        {label}
      </span>
      <div className={`h-px flex-1 border-t border-dashed ${lineColor}`} />
    </motion.div>
...
export default function PlayersList({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { success, participantsData, isActivePlayer } =
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <motion.div
          initial="hidden"
          animate="visible"
          className="grid grid-cols-4 gap-x-4 gap-y-4 xs:grid-cols-5 sm:grid-cols-6"
        >
          <AnimatePresence>
            {players &&
              players.map((player, index) => {
                ...
                return (
                  <Fragment key={player.id}>
                    {showMinMarker && (
                      <MilestoneMarker label={`${minPlayers} minimum`} variant="success" />
                    )}
                    {showMaxMarker && (
                      <MilestoneMarker label={`${maxPlayers} full`} variant="warning" />
                    )}
                    <motion.div
                      custom={index}
