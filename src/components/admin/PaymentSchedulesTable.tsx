      ...
      {filteredSchedules.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {filteredSchedules.map(schedule => {
            ...
            return (
              <div key={schedule.id} className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700/60 dark:bg-zinc-900">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-zinc-800 dark:text-zinc-100">
                      {schedule.membership.user.given_name} {schedule.membership.user.family_name}
                    </p>
                    <p className="text-xs text-zinc-400 dark:text-zinc-500">{schedule.membership.user.email}</p>
                  </div>
                  <span
                ...
                <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-zinc-100 pt-2.5 text-[11px] tabular-nums dark:border-zinc-800">
                  <span className="text-zinc-400 dark:text-zinc-500">{schedule.league.name}</span>
                  <span className="text-zinc-400 dark:text-zinc-500">Due {new Date(schedule.due_date).toLocaleDateString()}</span>
                  {schedule.status !== 'PAID' && (
                    <span className={daysUntil < 0 ? 'text-red-500' : daysUntil < 7 ? 'text-basket-400' : 'text-zinc-400 dark:text-zinc-500'}>
                      {daysUntil < 0 ? `${Math.abs(daysUntil)}d overdue` : `${daysUntil}d left`}
                    </span>
                  )}
                  <span className="font-medium text-zinc-700 dark:text-zinc-300">{formatCurrency(schedule.amount_due)}</span>
                  {schedule.amount_paid > 0 && <span className="text-green-500">{formatCurrency(schedule.amount_paid)} paid</span>}
                </div>
