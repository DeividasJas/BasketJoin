...
const statusStyle: Record<string, string> = {
  ...
  PENDING_PAYMENT: 'bg-yellow-50 text-yellow-600 dark:bg-yellow-500/10 dark:text-yellow-400',
  EXPIRED: 'bg-zinc-100 text-zinc-500 dark:bg-zinc-500/10 dark:text-zinc-400',
  CANCELLED: 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400',
}

export default function MembershipsList({ leagueId, memberships }: MembershipsListProps) {
  const [searchTerm, setSearchTerm] = useState('')

  const filteredMemberships = memberships.filter(membership => {
    const fullName = `${membership.user.given_name} ${membership.user.family_name}`.toLowerCase()
  ...
  const getPaymentStatus = (schedules: Membership['payment_schedules']) => {
    ...
    const hasOverdue = schedules.some(s => s.status === 'OVERDUE' || s.status === 'PENDING')
    if (hasOverdue) {
  ...
  return (
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-[11px] font-semibold uppercase tracking-[0.15em] text-zinc-500">Members ({memberships.length})</p>
        <input
          type="text"
      ...
      {filteredMemberships.length === 0 ? (
        <div className="rounded-xl border border-dashed border-zinc-300 p-8 text-center dark:border-zinc-700">
          ...
          <p className="mt-2 text-sm text-zinc-400 dark:text-zinc-500">{searchTerm ? 'No members found' : 'No members yet'}</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {filteredMemberships.map(membership => {
            const paymentStatus = getPaymentStatus(membership.payment_schedules)

            return (
              <div key={membership.id} className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700/60 dark:bg-zinc-900">
