...
export default function RescheduleForm({ gameId, currentDate, playerCount }: { gameId: number; currentDate: Date; playerCount: number }) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [newDate, setNewDate] = useState(new Date(currentDate).toISOString().slice(0, 16))
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700/60 dark:bg-zinc-900">
      <div className="rounded-lg border border-basket-400/20 bg-basket-400/5 p-3.5">
        <p className="text-xs text-zinc-600 dark:text-zinc-400">
          This will notify all <strong className="text-zinc-800 dark:text-zinc-200">{playerCount} registered players</strong> about the schedule change.
        </p>
      </div>
