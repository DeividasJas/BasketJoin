 ...
 */
export function generateRecurringDates(startDate: Date, endDate: Date, pattern: RecurringPattern): Date[] {
  const dates: Date[] = []
...
export function validateScheduleConfig(
  ...
  if (scheduleType === 'RECURRING') {
    ...
    if (
      ...
      (recurringPattern.dayOfWeek === undefined || recurringPattern.dayOfWeek < 0 || recurringPattern.dayOfWeek > 6)
    ) {
      return {
        isValid: false,
...
export function formatScheduleDescription(league: League): string {
  if (league.schedule_type === 'RECURRING' && league.recurring_pattern) {
    try {
      ...
      const frequencyMap = {
        DAILY: 'Daily',
        WEEKLY: pattern.dayOfWeek !== undefined ? `Every ${dayNames[pattern.dayOfWeek]}` : 'Weekly',
        MONTHLY: 'Monthly',
      }
 ...
 */
export function parseRecurringPattern(recurringPatternJson: string): RecurringPattern | null {
  try {
